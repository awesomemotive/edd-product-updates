...
emailTestRedirect();function emailConfirmPreview(){var button=$('#send-prod-updates'),spinner=$('.edd-pup-spin'),tinymce=$('#edd_pup_tinymce_status').val();button.mousedown(function(){if(tinymce=='true'){tinyMCE.triggerSave();}}).click(function(){var url=document.URL,form=$('#edd-pup-email-edit').serialize(),data={'action':'edd_pup_confirm_ajax','form':form,'url':url};if(emailValidate($('#from_email').val())){$(this).prop("disabled",true);spinner.toggleClass('loading');$.post(ajaxurl,data).error(function(){alert(eddPup.a1);spinner.toggleClass('loading');button.prop("disabled",false);}).success(function(r){if(r=='nocheck'){alert(eddPup.a3);spinner.toggleClass('loading');button.prop("disabled",false);}else if(r=='noncefail'){alert(eddPup.a7+' Nonce failure A.');spinner.toggleClass('loading');button.prop("disabled",false);}else if(r%1==0){var u=url.replace('add_pup_email','edit_pup_email');window.location.href=u+'&id='+r+'&edd_pup_confirm=1';}else{$.colorbox({html:r});spinner.toggleClass('loading');button.prop("disabled",false);}});return false;}else{alert(eddPup.a2);return false;}});}
