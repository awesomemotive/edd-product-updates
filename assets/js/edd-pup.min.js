...
eddPupRemoveMenus();$('#cboxContent .closebutton').live('click',function(){$.fn.colorbox.close();});$('#edd-pup-single-email .message-toggle').click(function(){if($(this).hasClass('active')){return false;}else{if($(this).data('message')=='preview'){$('#message-preview').show();$('#message-original').hide();}else if($(this).data('message')=='original'){$('#message-original').show();$('#message-preview').hide();}else{return false;}
$('#message-preview','#message-original').toggle();$('.message-toggle').toggleClass('active');return false;}});if($('#edd-pup-queue-details').length){$('#edd-pup-view-queue-alert').colorbox({inline:true,href:$('#edd-pup-queue-details'),width:'95%',maxWidth:'680px',height:'auto'});}
var url=document.URL;$('.edd-pup-queue-button').click(function(){var data={'action':$(this).attr('data-action'),'email':$(this).attr('data-email'),'url':$(this).attr('data-url'),'nonce':$(this).attr('data-nonce')};if(data['action']=='edd_pup_clear_queue'){if(confirm(eddPup.c1)){$.post(ajaxurl,data).error(function(){alert(eddPup.a9);}).success(function(response){if(response=='noncefail'){alert('Nonce failure: '+eddPup.a9);return false}
...
function emailPreview(){var button=$('#edd-pup-open-preview'),tinymce=$('#edd_pup_tinymce_status').val();button.mousedown(function(){if(tinymce=='true'){tinyMCE.triggerSave();}}).click(function(){if(tinymce=='true'){tinyMCE.triggerSave();}
var url=document.URL,form=$('#edd-pup-email-edit').serialize(),data={'action':'edd_pup_ajax_preview','form':form,'url':url,'nonce':$('#edd-pup-prev-nonce').val()};$.post(ajaxurl,data).error(function(){alert(eddPup.a1);button.prop("disabled",false);}).success(function(response){if($.isNumeric(response)){var u=url.replace('add_pup_email','edit_pup_email');window.location.href=u+'&id='+response+'&edd_pup_preview=1';}else{$.colorbox({html:response});}});});}
...
emailPreviewRedirect();function emailTest(){var button=$('#edd-pup-send-test'),tinymce=$('#edd_pup_tinymce_status').val();button.mousedown(function(){if(tinymce=='true'){tinyMCE.triggerSave();}}).click(function(){if(tinymce=='true'){tinyMCE.triggerSave();}
var url=document.URL,form=$('#edd-pup-email-edit').serialize(),data={'action':'edd_pup_send_test_email','form':form,'url':url};button.attr("disabled",true);if(emailValidate($('#from_email').val())){$.post(ajaxurl,data).error(function(){alert(eddPup.a1);button.attr("disabled",false);}).success(function(response){if($.isNumeric(response)){var u=url.replace('add_pup_email','edit_pup_email');window.location.href=u+'&id='+response+'&edd_pup_test=1';}else{alert(response);button.attr("disabled",false);}});}else{button.attr("disabled",false);alert(eddPup.a2);return false;}});}
...
$.post(ajaxurl,data).error(function(){alert(eddPup.a6);button.prop('disabled',false).attr({'data-action':'start',value:eddPup.v1});spinner.hide();}).success(function(ret){if(ret=='noncefail'){alert(eddPup.a7+' Nonce failure B.');button.prop('disabled',false)
spinner.hide();return false;}
if(ret=='usersendfail'){alert(eddPup.a11);button.prop('disabled',false);if(button.data('action')!='start'){button.attr({'data-action':'resume',value:eddPup.v3});clock.timer('pause');}
spinner.hide();return false;}
var r=$.parseJSON(ret),p=Math.round((r.sent/r.total)*100);$('.progress-wrap').css('opacity','1');$('.progress-queue').text(prettyNumber(r.total));button.prop('disabled',false).attr({'data-action':'pause',value:eddPup.v2});spinner.hide();if(r.status=='restart'){psent.text(prettyNumber(r.sent));bar.attr('data-complete',p).css('width',p+'%');pperc.text(p+'%');eddPupAjaxTrigger(i,r.sent,r.total,emailid,0);}else{eddPupAjaxBuild(r,emailid,1,0);}
...
var data={'action':'edd_pup_ajax_start','email_id':emailid,'iteration':it,'processed':r.processed,'status':'processing','nonce':nonce,'error':err};$.post(ajaxurl,data).error(function(){err++;status.html(eddPup.s7);var errsec=14,errtimer=setInterval(function(){$('.progress-status .count').text(errsec--);if(errsec==0){clearInterval(errtimer);}
...
if(+i==0&&s>0&&paused==0){$('.success-restart-p').text(s);}
...
var err=0;if(!$.isNumeric(s)){if(s=='epat_id_err'){alert(eddPup.a7+' Email ID undefined.');}else if(s=='epat_res_err'){alert(eddPup.a7+' MYSQL results error.');}else if(s=='epat_up_err'){alert(eddPup.a7+' MYSQL update error.');}else{alert(eddPup.a7+' Numeric failure.');}
clock.timer('pause');if(button.attr('data-action')!='resume'){button.attr({'data-action':'resume',value:eddPup.v3});paused++;}
return false;}
